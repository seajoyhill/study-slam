// 系统已安装 ROS1（rosbag、sensor_msgs、pcl_conversions）与 PCL 时由 CMake 编译。
// 用法: rosbag_pcl_smoke_test <file.bag> [pointcloud_topic]
// 未指定 topic 时处理 bag 内所有 sensor_msgs/PointCloud2。
#include <iostream>
#include <string>
...
int main(int argc, char** argv) {
  ...
  for (const rosbag::MessageInstance& m : view) {
    ...
    if (n_msg <= 5) {
      std::cout << "msg " << n_msg << " topic=" << m.getTopic() << " width=" << cloud->width
                << " height=" << cloud->height << " pcl_points=" << pcl_cloud.size() << '\n';
    }
  }
