// Demo: read /odom translation from rosbag and visualize rslidar(frame from
// /velodyne_points) moving in map frame with Pangolin. Orientation is
...
void DrawRosAxis(float s) {
    ...
    pangolin::glDrawColoredVertices<float, float>(6, lines, colors, GL_LINES, 3,
                                                  3);
}

std::vector<float> BuildTrajectoryVertices(
...
void DrawTrajectory(const std::vector<float>& vertices, std::size_t end_idx) {
    ...
    pangolin::glDrawVertices<float>(static_cast<GLsizei>(draw_points),
                                    vertices.data(), GL_LINE_STRIP, 3);
}

void DrawPointCloud(const std::vector<float>& xyz,
    ...
    pangolin::glDrawColoredVertices<float, float>(
        static_cast<GLsizei>(xyz.size() / 3), xyz.data(), rgb.data(), GL_POINTS,
        3, 3);
}

const sensor_msgs::PointField* FindIntensityField(
...
CloudSample BuildCloudSample(const sensor_msgs::PointCloud2& cloud,
                             std::size_t cloud_seq) {
    CloudSample sample;
    ...
    const std::size_t step = std::max<std::size_t>(
        1, total_pts / std::max<std::size_t>(1, keep_pts));
    ...
    const std::size_t intensity_offset =
        has_intensity ? intensity_field->offset : 0;
    const uint8_t intensity_datatype =
        has_intensity ? intensity_field->datatype : 0;
    ...
    for (std::size_t i = 0; i < total_pts; ++i, ++it_x, ++it_y, ++it_z) {
        ...
        float inten = 0.0f;
        if (has_intensity &&
            (i * cloud.point_step + intensity_offset) < cloud.data.size()) {
            const uint8_t* ptr =
                &cloud.data[i * cloud.point_step + intensity_offset];
    ...
    if (!sample.xyz.empty()) {
        ...
        const float denom =
            (norm_max > norm_min) ? (norm_max - norm_min) : 1.0f;
        ...
        // if (kEnableIntensityDebugLog && has_intensity && (cloud_seq <= 3 ||
        // cloud_seq % 100 == 0)) {
        //   const float blue_ratio = static_cast<float>(n_blue) /
        //   static_cast<float>(intensities.size()); const float red_ratio =
        //   static_cast<float>(n_red) / static_cast<float>(intensities.size());
        //   std::cout << "[intensity dbg] cloud=" << cloud_seq << " n=" <<
        //   intensities.size()
        //             << " raw[min,max]=(" << i_min << ", " << i_max << ")"
        //             << " norm[min,max]=(" << norm_min << ", " << norm_max <<
        //             ")"
        //             << " blue_ratio=" << blue_ratio << " red_ratio=" <<
        //             red_ratio << '\n';
        // }
    }
    return sample;
...
bool ReadTrajectory(const std::string& bag_path, const std::string& odom_topic,
                    const std::string& velodyne_topic,
    ...
    for (const rosbag::MessageInstance& m : view) {
        if (m.getTopic() == velodyne_topic &&
            m.getDataType() == "sensor_msgs/PointCloud2") {
            sensor_msgs::PointCloud2::ConstPtr cloud =
                m.instantiate<sensor_msgs::PointCloud2>();
            if (cloud) {
                ++n_cloud;
                if (rslidar_frame_id->empty() &&
                    !cloud->header.frame_id.empty()) {
        ...
        if (m.getDataType() == "nav_msgs/Odometry") {
            ...
            poses->push_back(
                PoseSample{odom->header.stamp.toSec(), static_cast<float>(p.x),
        ...
        if (m.getDataType() == "geometry_msgs/PoseStamped") {
            ...
            poses->push_back(
                PoseSample{pose->header.stamp.toSec(), static_cast<float>(p.x),
                           static_cast<float>(p.y), static_cast<float>(p.z),
                           static_cast<float>(q.x), static_cast<float>(q.y),
                           static_cast<float>(q.z), static_cast<float>(q.w)});
            ++n_pose;
    ...
    std::cout << "Loaded odom samples from " << odom_topic << ": "
              << poses->size() << " (Odometry=" << n_odom
              << ", PoseStamped=" << n_pose << ")\n";
    std::cout << "Loaded point clouds from " << velodyne_topic
              << ": total=" << n_cloud << ", kept=" << n_cloud_kept
              << ", stride=" << kCloudFrameStride
    ...
    if (!rslidar_frame_id->empty()) {
        std::cout << "Detected " << velodyne_topic
                  << " frame_id: " << *rslidar_frame_id << '\n';
    } else {
...
int main(int argc, char** argv) {
    ...
    const std::string velodyne_topic =
        (argc >= 4) ? argv[3] : std::string("/velodyne_points");

    std::vector<PoseSample> poses;
    std::vector<CloudSample> clouds;
    std::string rslidar_frame_id;
    if (!ReadTrajectory(bag_path, odom_topic, velodyne_topic, &poses, &clouds,
                        &rslidar_frame_id)) {
        std::cerr << "No valid odom translation samples found on topic "
                  << odom_topic << ".\n";
        return 2;
    }
    const std::vector<float> trajectory_vertices =
        BuildTrajectoryVertices(poses);
    ...
    pangolin::OpenGlRenderState s_cam(
        pangolin::ProjectionMatrix(w, h, 700, 700, w / 2.0, h / 2.0, 0.1,
                                   10000),
        pangolin::ModelViewLookAt(-20, -20, 12, 0, 0, 0, pangolin::AxisZ));

    pangolin::Handler3D handler(s_cam, pangolin::AxisNone,
                                kCameraTranslationScale, kCameraZoomFraction);
    pangolin::View& d_cam =
    ...
    const auto frame_interval =
        std::chrono::duration<double>(1.0 / kTargetFrameRate);
    const auto play_start = std::chrono::steady_clock::now();
    auto next_frame_time = play_start;

    std::cout << "Playing map <- " << rslidar_frame_id << " translation from "
              << odom_topic << ". Controls: mouse drag/zoom, ESC to exit.\n";

    while (!pangolin::ShouldQuit()) {
        const auto now = std::chrono::steady_clock::now();
        double play_sec =
            std::chrono::duration<double>(now - play_start).count() * speed;
        if (duration > 0.0 && play_sec > duration) {
            play_sec = duration;
        }

        while (idx + 1 < poses.size() &&
               (poses[idx + 1].t_sec - t0) <= play_sec) {
            ++idx;
        }
        while (cloud_idx + 1 < clouds.size() &&
               clouds[cloud_idx + 1].t_sec <= poses[idx].t_sec) {
            ++cloud_idx;
        }
        ...
        ApplyQuaternionRotation(poses[idx].qx, poses[idx].qy, poses[idx].qz,
                                poses[idx].qw);
        if (!clouds.empty()) {
