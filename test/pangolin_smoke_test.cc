// Pangolin 烟测：交互 3D 视图 + RGB 轴线（线框箭头）+ 轴上文字（P*MV 投到 View
// 像素坐标后 Draw）。
...
void GlDrawAxisWithArrows(float s) {
    ...
    auto lines = [&](const float* v, float r, float g, float b) {
        ...
        pangolin::glDrawColoredVertices<float, float>(6, v, cols, GL_LINES, 3,
                                                      3);
    };
    const float vx[] = {0, 0,   0, s, 0, 0, s, 0,    0,
                        t, rad, 0, s, 0, 0, t, -rad, 0};
    lines(vx, 1, 0, 0);
    const float vy[] = {0,   0, 0, 0, s, 0, 0,    s, 0,
                        rad, t, 0, 0, s, 0, -rad, t, 0};
    lines(vy, 0, 1, 0);
    const float vz[] = {0,   0, 0, 0, 0, s, 0,    0, s,
                        rad, 0, t, 0, 0, s, -rad, 0, t};
    lines(vz, 0, 0, 1);
}

// 世界点 -> 当前 glViewport 内像素（相对 View 左下），用 cam 的 P*MV（勿用
// GlText::DrawWindow）
bool WorldToViewLocalPx(const pangolin::OpenGlRenderState& cam, float wx,
                        float wy, float wz, float* lx, float* ly) {
...
void DrawAxisLabels(pangolin::View& view,
                    const pangolin::OpenGlRenderState& cam,
                    const pangolin::GlText& tx, const pangolin::GlText& ty,
                    const pangolin::GlText& tz, float u, float off) {
    float lx[3], ly[3];
...
int main() {
    ...
    std::cout << "pangolin smoke: RGB 轴 + 标注，拖拽旋转、滚轮缩放，关窗或 "
                 "ESC 退出\n";
