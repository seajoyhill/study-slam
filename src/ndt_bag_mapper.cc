// 极简激光建图：从 rosbag 读 PointCloud2，用相邻两帧 NDT 估计位姿，累加并保存
// PCD。
...
int main(int argc, char** argv) {
    if (argc < 3) {
        std::cerr << "Usage: " << argv[0]
                  << " <file.bag> <out.pcd> [pointcloud_topic]\n";
        return 1;
    }
    ...
    for (const rosbag::MessageInstance& m : view) {
        ...
        if (have_prev) {
            ...
            if (target_ds.size() < 50 || source_ds.size() < 50) {
                std::cerr << "frame " << frame
                          << ": too few points after downsample, skip NDT\n";
            } else {
                ...
                if (!ndt.hasConverged()) {
                    std::cerr << "frame " << frame
                              << ": NDT not converged, use identity\n";
                } else {
        ...
        if (frame <= 5 || frame % 50 == 0) {
            std::cout << "frame " << frame << " topic=" << m.getTopic()
                      << " map_pts=" << map_global.size() << '\n';
        }
    }
    ...
    std::cout << "saved " << out_pcd << " points=" << map_global.size()
              << " frames=" << frame << '\n';
    return 0;
}
